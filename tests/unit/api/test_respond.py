...
def test_respond_observables_call_success(
        client, valid_jwt,
        akamai_response_network_lists, respond_observables_expected_payload
):
    with patch.object(Session, 'request') as request_mock:
        ...
        request_mock.assert_called_once_with(
            ...
            params={'listType': 'IP', 'includeElements': True}
        )
...
def input_sets():
    ...
    TestData = namedtuple('TestData',
                          'test_name json expected_response request_check')
    yield TestData(
        'add_action_success',
        {**input_json, 'action-id': ADD_ACTION_ID},
        expected_response_success,
        lambda r: r.assert_called_once_with(
            'PUT', 'https://xxx/network-list/v2/network-lists/nli/elements',
            headers={'Accept': 'application/json',
                     'User-Agent': 'Cisco Threat Response Integrations'
                                   ' <tr-integrations-support@cisco.com>'},
    ...
    yield TestData(
        ...
        expected_response_success,
        lambda r: r.assert_called_once_with(
            'DELETE', 'https://xxx/network-list/v2/network-lists/nli/elements',
            headers={'Accept': 'application/json',
                     'User-Agent': 'Cisco Threat Response Integrations'
                                   ' <tr-integrations-support@cisco.com>'},
            params={'element': '1.1.1.1'}
...
def test_respond_trigger(input_data,
                         client, valid_jwt,
                         akamai_response_ok
                         ):
    with patch.object(Session, 'request') as request_mock:
